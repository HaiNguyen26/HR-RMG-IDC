// Script import ứng viên trên Local (Node.js - UTF-8 safe)
...
// Danh sách ứng viên (với ký tự tiếng Việt đầy đủ)
const candidates = [
    // Hình 1
...
for (const candidate of candidates) {
    ...
    if (!seenPhones.has(normalizedPhone)) {
        ...
        candidate.soDienThoai = normalizedPhone;
...
async function importCandidates() {
    try {
        ...
        // Kiểm tra tổng số ứng viên
        const totalResult = await pool.query('SELECT COUNT(*) as total FROM candidates');
        console.log(`Tổng số ứng viên trong database: ${totalResult.rows[0].total}`);
