        ...
        if (isHr) {
            ...
            const columnCheck = await pool.query(
                `SELECT column_name FROM information_schema.columns 
                 WHERE table_name = 'leave_requests' AND column_name = 'cancellation_reason'`
            );
            ...
            if (hasCancellationReason && cancellationReason) {
                await pool.query(
                    `UPDATE leave_requests SET status = $1, cancellation_reason = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3`,
