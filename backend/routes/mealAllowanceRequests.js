        ...
        if (isHr) {
            ...
            const columnCheck = await pool.query(
                `SELECT column_name FROM information_schema.columns 
                 WHERE table_name = 'meal_allowance_requests' AND column_name = 'cancellation_reason'`
            );
            ...
            if (hasCancellationReason && cancellationReason) {
                await pool.query(
                    `UPDATE meal_allowance_requests SET status = $1, cancellation_reason = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3`,
                    [STATUSES.CANCELLED, cancellationReason.trim(), parseInt(id, 10)]
                );
            } else {
                await pool.query(
                    `UPDATE meal_allowance_requests SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2`,
